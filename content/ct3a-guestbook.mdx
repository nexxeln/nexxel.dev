...
If you take a look at the `src/server/common/get-server-auth-session.ts` file, we're using [`unstable_getServerSession`] from Next Auth that gives us access to the session on the server. We're passing that into our tRPC context. We can use this session to make our mutation protected using a `protectedProcedure`.
...
Now, replace `publicProcedure` with `protectedProcedure` to make our mutation protected.
