...
const shareOnTwitter = () => {
  openInNewTab(`https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}`);
};
...
const shareOnReddit = () => {
  openInNewTab(`https://www.reddit.com/submit?url=${encodeURIComponent(url)}`);
};
...
const shareByEmail = () => {
  openInNewTab(`mailto:?subject=Check this out&body=${encodeURIComponent(url)}`);
};
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button onClick={handleCopy} type="button" aria-label="Copy link">
        <svg className="w-4 h-4 text-[#0e0909] dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
      ...
      <button onClick={shareByEmail} type="button" aria-label="Copy link">
        <svg className="w-4 h-4 dark:text-white" width="24" height="24" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
      ...
      <button onClick={shareOnLinkedIn} type="button" aria-label="Copy link">
       ...
       <button onClick={shareOnReddit} type="button" aria-label="Copy link">
            <svg className="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium MuiSvgIcon-root MuiSvgIcon-fontSizeMedium svg-icon css-5zsjn4" width="16" height="16" fill="#ffffff" aria-hidden="true" viewBox="0 0 24 24"><path d="M22 12.14a2.19 2.19 0 0 0-3.71-1.57 10.93 10.93 0 0 0-5.86-1.87l1-4.7 3.27.71a1.56 1.56 0 1 0 .16-.76l-3.64-.77c-.11-.02-.22 0-.29.06-.09.05-.14.14-.16.26l-1.11 5.22c-2.33.07-4.43.78-5.95 1.86A2.2 2.2 0 0 0 4.19 10a2.16 2.16 0 0 0-.9 4.15 3.6 3.6 0 0 0-.05.66c0 3.37 3.92 6.12 8.76 6.12s8.76-2.73 8.76-6.12c0-.21-.01-.44-.05-.66A2.21 2.21 0 0 0 22 12.14M7 13.7c0-.86.68-1.56 1.54-1.56s1.56.7 1.56 1.56a1.56 1.56 0 0 1-1.56 1.56c-.86.02-1.54-.7-1.54-1.56m8.71 4.14C14.63 18.92 12.59 19 12 19c-.61 0-2.65-.1-3.71-1.16a.4.4 0 0 1 0-.57.4.4 0 0 1 .57 0c.68.68 2.14.91 3.14.91s2.47-.23 3.14-.91a.4.4 0 0 1 .57 0c.14.16.14.41 0 .57m-.29-2.56c-.86 0-1.56-.7-1.56-1.56a1.56 1.56 0 0 1 1.56-1.56c.86 0 1.58.7 1.58 1.56a1.6 1.6 0 0 1-1.58 1.56z"></path>
            </svg>
      ...
      <div className="relative" ref={moreRef}>
        ...
        {showMore && (
          <div className="absolute right-0 z-10 mt-2 w-40 rounded-md bg-white dark:bg-neutral-900 shadow-lg ring-1 ring-black ring-opacity-5 p-2 flex flex-col gap-1">
            {/* Whatsapp */}
            <button onClick={shareOnWhatsapp } className="flex items-center gap-2 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded px-2 py-1" type="button">
            ...
            {/* Facebook */}
            <button onClick={shareOnFacebook } className="flex items-center gap-2 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded px-2 py-1" type="button">
                <svg className="w-4 h-4 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
           ...
           {/* Telegram*/}
            <button onClick={shareOnTelegram } className="flex items-center gap-2 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded px-2 py-1" type="button">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"><path fill="#ffffff" d="m21.936 5.17l-3.03 14.185c-.226.999-.806 1.224-1.644.773l-4.545-3.352l-2.225 2.127c-.225.226-.451.452-.967.452l.355-4.675l8.478-7.704c.354-.355-.097-.484-.548-.193l-10.541 6.64l-4.546-1.386c-.999-.322-.999-1 .226-1.45L20.614 3.72c.87-.258 1.612.194 1.322 1.45"/></svg>
