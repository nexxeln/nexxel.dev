...
In the example above, you'll probably want to add some of your own functionality in the `middlware` function. For example, you might want to fetch the user making the request from your database, and then pass that data to onUploadComplete. I didn't show specifics here as the exact flow depends a lot on what your Schema and Auth solution and look like.

<div class="my-8"/>
Once we pass down our metadata, we can create a new Audio record in our DB that's attached to our user/project. The idea here is that we are creating the audio file in our database as soon as it's uploaded by the user so that we can update the UI to show the user their UploadedFile. We'll update this Audio record with the transcription once it's generated!
<div class="my-8"/>
...
• We use headers to pass in the projectId to our audioUploader endpoint, so that we can use the Id in our onUploadComplete callback to add the audio to our database

• Using Apperance, Content, and className, we customize the UI of the UploadThing component to be mobile friendly

• We use onClientUploadComplete to keep the UI responsive while the file upload is in progress, and when it completes. First we invalidate the getAudio query to force a re-fetch of the audio data.  Next we hit the /transcrbie endpoint(we're about to create this in the next step) to transcribe the audio. Once we sucessfully transcribe the audio, we invalidate the getAudio query again to force a re-fetch of the audio data, this time with the transcription.

## <div class="text-2xl">Creating the /transcribe endpoint</div>

Ok so now the fun part! First thing's first, we'll create a new folder in our `src/app/api/` directory called `transcribe`. Inside this folder, we'll create a new file called `route.ts`.
...
const streampot = new StreamPot({
 ...
 const chunkJob = await streampot
  ...
  if (chunkJob.status === "completed") {
    console.log("completed", chunkJob);
